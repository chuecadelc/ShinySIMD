...
countriesGla <- readOGR("SG_SIMD_2016_3.geojson", "SG_SIMD_2016_2", stringsAsFactors = F)
    ...
    tabItems(
      tabItem(tabName = "Info",
              ...
              p("This app will be using the Scottish Index of Multiple Deprivation to showcase measures of central tendency, statistical tests such as correlation.
                 In terms of visualisations it haves scatterplots, histograms, and an interactive map of Glasgow.",style = "font-size:28px"),
              br(),
              p("The aim of this app is to explain the different relationships between the variables in such index, obtained from the Scottish Government Data Center.",style = "font-size:28px"),
              br(),
              p("Developed by Cristina Chueca, Q-Step Graduate of the University of Glasgow 2018, in collaboration with Dr. Brian Fogarty and Dr. Niccole Pamphilis.",style = "font-size:28px"),
              br()
             ),
      #tabsetPanel(
      tabItem(tabName = "Stats",
        fluidRow(
          ...
          box(width = 3,
              sidebarPanel(
                selectInput("cov3", "Variables:",
                            choices = names(data2), selected = "Population"),width = 11)),
          box(width = 2,
              selectInput("bincolor","Select a Color",
                          choices = colors(), selected = "grey"),
              
              checkboxInput("addmean", "Add the mean line?", value = FALSE)),
          
          box(width= 7, sliderInput("bins", "Number of bins:",
      ...
      tabItem(tabName = "Comparison",
              fluid = TRUE,
              fluidRow(
                box(width=5, selectInput("cov1", "X-Axis", choices = names(data2)),
                    selectInput("cov2", "Y-Axis", choices = names(data2)),
                    
                    verbatimTextOutput("Explanation1"),
                    
                    verbatimTextOutput("Explanation2"),
                    
                    checkboxInput("gla", "What about only in Glasgow?",
                                  value = FALSE)),
                
                box(selectInput("bincolor1", "Color",
                                choices = colors(),
                                selected = "grey"),
                    
                    checkboxInput("addcor","Add Pearson's correlation to the graph?",  
                                  value = FALSE)),
                box(width= 7,
                    mainPanel(title = "Scatterplot", 
                              status = "primary",
                              solidHeader = TRUE, 
      ...
      tabItem(tabName = "map", 
              fluidRow( width = 6,
                       ...
                       sidebarPanel(
                         selectInput("cov4", "Variables:", 
                                     choices = names(data2)), selected = "Population",
...
server <- function(input, output, session) {
  ...
  output$distPlot <- renderPlot({
    ...
    if (input$addmean == "TRUE") {
      ...
      + geom_vline(xintercept = mean(data2[,input$cov3]), lwd = 2, =lty = 2)
